...
When encounting some garbled characters encoding problem, you must want to convert it to the correct encoding format, linux provides `iconv` command to convert file beetween different character encodings. But the from-encoding and to-encoding must be specified and it may still generate some garbled characters like **'?? ¨ª¡¤2?¡¤¡§'**. 
